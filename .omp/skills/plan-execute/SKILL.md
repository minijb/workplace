...
> **plan.status 闸门**：若 `plan.status: blocked`，**不再取新 leaf**（见 §7 退出条件）。下游受阻 leaf（`depends` 指向某个 blocked leaf）保持 `todo`；重试期内（replan §1 的 ≤2 次内）的 leaf 保持 `in_progress`。

**Dotted ID 解析**：从计划根起，按 ID 段匹配对应序号的子节点（`1.2` → 根的第 1 个子节点 → 其第 2 个子节点），定位到 leaf 文件或 group 目录。**frontmatter `task.id` 为声明真源，路径序号用于一致性校验**——两者不一致即计划违规，标 `blocked` 并交还 `plan-create`，不在违规计划上硬跑。
...
若无就绪 leaf 且有 `in_progress` → 继续该 leaf。**subagent/hybrid 下存在多个 `in_progress` 时，按 `modes.md` 波次协议续完本波**（barrier 内的并行 leaf 继续推进至本波全部 done/blocked）；无法续（波次中断且无明确续期路径）则进 §5。若无就绪且无 `in_progress`、还有 `todo` → 前置链断了，进 §5 replan 或标 `blocked`。

## 3. 执行单 leaf（react / 单 leaf 视角）

不论整体模式，**单个 leaf** 的执行循环节奏：

1. **改状态**：`read` 该 leaf 文件，把 `status` 从 `todo` 改 `in_progress`；刷新 `plan.md` 的 `updated`；`write` 回该 leaf 文件（写全文件）。**首次**把任一 leaf 置 `in_progress` 时（即本计划首次进入执行），同步把 `plan.status: ready → in_progress`（与 step5 一致，刷新顶层 INDEX 状态列）。
...
5. **改状态 + 刷新 roll-up**：`status: done`；刷新 `plan.md` 的 `updated`；`write` 该 leaf 文件；然后**向上刷新 roll-up**——重算每个祖先 group `INDEX.md` 的派生状态行与进度（规则见 `plan-format.md`「状态词表」），并刷新顶层 `docs/plan/INDEX.md` 该计划行的**进度列与状态列**（状态列镜像 `plan.status`，进度列 = done 叶子数 / 总叶子数）。
...
leaf 失败、验证反复不过、发现计划缺陷（acceptance 不可达、依赖缺失、files 漏标关键文件）时，**不要硬冲**。**Replan 规程见 `references/replan.md`——出错时必须 `read` 全文**；下面的决策树只是**备忘速览**，分支判据（尤其 §2 vs §3）以 replan.md 详文为准。

```
leaf 失败
├─ 可在原 acceptance 内修 → 修，重试 ≤2 次（不计数）
├─ acceptance 本身有缺陷/不可达，但 leaf 目标仍合理、leaf 边界不变 → 局部 replan：改该 leaf 的 acceptance/verification（不动树结构）
├─ 计划级缺陷（依赖断裂、目标不可达、需拆 leaf）→ 标 plan.status: blocked，向用户报告
└─ 触发收敛上限（局部 replan 计数 ≥3 次仍未收敛）→ 标 blocked
```

**局部 replan 计数（per-plan）**：executor 每做一次局部 replan（§2）自增 `replan_count`（字段载体见 `plan-format.md` plan frontmatter），**scope 是单个计划**，不是 per-leaf。≥3 次仍未向 done 收敛 → 标 `plan.status: blocked`。每次局部 replan 在「复盘笔记」段的条目**必须带 `(n/3)` 计数**（如 `(1/3)`、`(2/3)`）；`blocked`/非 replan 条目不带计数。

replanned 后必须刷新 `plan.md` 的 `updated`、自增 `replan_count`，并在「复盘笔记」段记一笔（改了哪个 leaf、改了什么、为什么、计数）。
...
4. **归档**：把**整个计划目录**从 `docs/plan/active/` 移到 `docs/plan/completed/`；把顶层 `docs/plan/INDEX.md` 该行从「进行中」段挪到「已完成」段——**两段列结构不同**（进行中 6 列 / 已完成 4 列），挪行即**丢弃状态/进度/模式列**，补上**完成日与一句话结果列**。
