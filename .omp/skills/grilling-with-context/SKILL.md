...
1. **检查 docs 工作区**：`glob` 确认 `docs/` 及其 `INDEX.md`、各子目录 `INDEX.md` 是否就位（由 `docs-workspace` 搭建）。**未就位** → 告知用户先跑 `docs-workspace`；本 skill 仍可懒创建根级 `CONTEXT.md`，但不负责补全套脚手架。**目录兜底总则**：当盘问结晶出该进 `docs/spec`、`docs/adr`、`docs/reference` 的知识、而对应目录不存在时——可为本 skill 即将写入的**单个**目标目录懒创建该目录 + 其同级 `INDEX.md` 一行，并在父级 `docs/INDEX.md`（不存在则一并建）登记该新子目录一行。**不**批量初始化全部目录、**不**配 `.gitignore`——那些归 `docs-workspace`。
...
4. **读相关 ADR**：经 `docs/adr/INDEX.md` 找到与本次设计相关的 ADR 并读，避免重复决策或与既有决策冲突。**INDEX / 目录缺失降级**：`docs/adr/INDEX.md` 或 `docs/adr/` 不存在 → `glob docs/adr/*.md` 兜底查重；仍无任何 ADR → 记「无既有 ADR」继续，不阻塞盘问。
...
- **遍历设计树**：沿设计的每个分支逐题推进，逐个理清决策间的依赖——先问被依赖最多的底层概念。分支发现至少覆盖六类（缺一类即未穷尽）：**①概念定义**（每个核心概念是什么/不是什么）、**②关系模型**（概念间如何关联/组合/引用）、**③生命周期与可变性**（创建/流转/销毁、可变/不可变）、**④存放与索引**（落在哪里、如何被检索）、**⑤命名碰撞**（同名异义/异名同义、与既有术语冲突）、**⑥边缘场景**（空、极大、并发、失败回滚等边界）。**穷尽 = 六类各有结论，或用户对某类明确说「跳过」**。
...
- **拿具体场景施压**：讨论领域关系时，构造探测边界的具体场景（尤其边缘情况），逼用户把概念边界说精确。「你说一个 Order 只属于一个 Customer——那合并下单、子账户代付时 Order 归谁？」用这类问题逼出概念间的真实基数与归属。
...
| 目录 / 放置约定（非术语、非 spec、够不上 ADR） | 归属对应 `skills/lib/*-format.md` 格式契约；需新建目录骨架交 `docs-workspace`；当轮可暂记入 `CONTEXT.md` → `Open Questions` 但须标注「非术语-放置约定」 | 相关 `*-format.md` |
...
- **spec 够格才写**：仅当某分支已就接口/验收达成共识、且属非术语时才开 `docs/spec/`；尚未达成共识或仍属术语层面 → 留 `CONTEXT.md` → `Open Questions`，不抢开 spec。
- **ADR 克制**：仅当三条**同时**成立才提议 ADR（难回退 / 无背景会意外 / 真实权衡，详见 `adr-format.md`「何时创建」）。编号扫 `docs/adr/` 取现有最大 +1。三条缺一即跳过。**提议 → 落盘的 Status 流向**：①**提议** = 给建议编号 + 一句话摘要（口头，不写文件）；②**写入 `Status: proposed` 文件**须经用户首肯；③**用户确认** = 翻 `Status: accepted`。
- **同步 INDEX（写 `docs/` 文件时必做）**：在 `docs/` 下新增/删除任何文件，立即按 `index-format.md` 更新**同级** `INDEX.md` 一行说明——不积压。`CONTEXT.md` 在根或模块目录、不属 `docs/`，无需进 INDEX。
- **同步 CONTEXT-MAP（多上下文时）**：若盘问中**浮现一个新上下文**并新建了它的 `CONTEXT.md`，按 `context-format.md` 把它登记进根级 `CONTEXT-MAP.md` 的 `Contexts`，必要时补 `Relationships`。**首次发现多上下文（此前按单上下文、用根级 `CONTEXT.md` 走到现在）的迁移步骤**：①建根级 `CONTEXT-MAP.md`（按 `context-format.md`）；②把根级 `CONTEXT.md` 中**模块专属术语**迁入对应模块的 `CONTEXT.md`；③根级 `CONTEXT.md` 只留**跨上下文通用**术语（必要时 `See also` 互链）。

## 4. 收敛

- 每解决一个分支，简述结论以及它如何（或无需）改动领域模型与 docs。
- 全部分支走完、与用户确认共识后，给出本次会话**对 docs 系统变更的摘要**：新增/修订了哪些术语与关系、哪些未决术语进了 Open Questions、新建了哪些 ADR / spec / reference、更新了哪些 INDEX / CONTEXT-MAP。**摘要模板（一行）**：`术语 +N 修订 M｜关系 +N｜Open Questions +K｜ADR 提议 A 落盘 B｜spec/reference +C｜INDEX/CONTEXT-MAP 更新 D`。
